...
int main( int argc, char* argv[] )
{
  ...
  // name of the executable (in our case, "a4").
    ...
    bool simple = true; //toggle between simple, or with the blurring etc.
    bool edge = false;
    bool dof = false;
    float focal_length = 1.5;
    float focal_num = 0.05;
    ...
    if (simple) {
        ...
        for (int i = 0; i < width; i++) {
            ...
            for (int j = 0; j < height; j++) {
                ...
                latest_column.push_back(pixels[lefti][topj]*-1.0f + pixels[lefti][centerj]*-2.0f + pixels[lefti][botj]*-1.0f +
                    //pixels[centeri][topj] + pixels[centeri][centerj] + pixels[centeri][botj]+ //we don't add this for left right edge.
                    pixels[righti][topj] + pixels[righti][centerj]*2.0f + pixels[righti][botj]
                );

            }
            gaussianWpixel.push_back(latest_column);
        }
        cout << "width done" << endl;
        ...
        for (int i = 0; i < width; i++) {
            ...
            for (int j = 0; j < height; j++) {
                ...
                latest_column.push_back(pixels[lefti][topj] * -1.0f + pixels[lefti][centerj] *0+ pixels[lefti][botj] +
                    pixels[centeri][topj]*-2.0f + pixels[centeri][centerj]*0 + pixels[centeri][botj]*2.0f+ 
                    pixels[righti][topj] * -1.0f + pixels[righti][centerj] * 0 + pixels[righti][botj]
                );
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                image.SetPixel(i, j, gaussianHpixel[i][j]+gaussianWpixel[i][j]);
            }
        }
    }
    else if(dof){
        vector<vector<float>> dofs;
        vector<vector<Vector3f>> pixels;
        for (int widthi = 0; widthi < width * 3; widthi++) {
            ...
            for (int heighti = 0; heighti < height * 3; heighti++) {
                ...
                Ray newray = camera->generateRay(Vector2f(screen_w_pixel, screen_h_pixel)); //shoot a ray out.
                Vector3f newpixelColor = rayTracer.traceRay(newray, camera->getTMin(), max_bounces, 1.0f, newhit);
                holder.push_back(rayTracer.depthshot(newray, focal_length, focal_num));

                //image.SetPixel(widthi, heighti, newpixelColor);
                latest_column.push_back(newpixelColor);
            }
            pixels.push_back(latest_column);
            dofs.push_back(holder);
        }
        ...
        for (int i = 0; i < width * 3; i++) {
            ...
            for (int j = 0; j < height * 3; j++) {
                if (j == 0) {
                    latest_column.push_back(pixels[i][j] * 1/4+
                        pixels[i][j] * 1 / 4 +
                        pixels[i][j] * 1 / 4 +
                        pixels[i][j + 1] * 1 / 4 +
                        pixels[i][j + 2] * 1 / 4);
                    ...
                    latest_column.push_back(pixels[i][j - 1] *1/ 4 +
                        pixels[i][j - 1] * 1 / 4 +
                        pixels[i][j] * 1 / 4 +
                        pixels[i][j + 1] * 1 / 3 +
                        pixels[i][j + 2] * 1 / 3);
                    ...
                    latest_column.push_back(pixels[i][j - 2] * 1 / 3 +
                        pixels[i][j - 1] * 1 / 3 +
                        pixels[i][j] * 1 / 3 +
                        pixels[i][j + 1] * 1 / 3 +
                        pixels[i][j + 1] * 1 / 3);
        ...
        cout << "height done" << endl;
        //downsample
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                Vector3f averageblur = gaussianWpixel[i * 3][j * 3] + gaussianWpixel[i * 3 + 1][j * 3] + gaussianWpixel[i * 3 + 2][j * 3] +
                    gaussianWpixel[i * 3][j * 3 + 1] + gaussianWpixel[i * 3 + 1][j * 3 + 1] + gaussianWpixel[i * 3 + 2][j * 3 + 1] +
                    gaussianWpixel[i * 3][j * 3 + 2] + gaussianWpixel[i * 3 + 1][j * 3 + 2] + gaussianWpixel[i * 3 + 2][j * 3 + 2];
                averageblur = averageblur / 9.0f;

                Vector3f average = pixels[i * 3][j * 3] + pixels[i * 3 + 1][j * 3] + pixels[i * 3 + 2][j * 3] +
                    pixels[i * 3][j * 3 + 1] + pixels[i * 3 + 1][j * 3 + 1] + pixels[i * 3 + 2][j * 3 + 1] +
                    pixels[i * 3][j * 3 + 2] + pixels[i * 3 + 1][j * 3 + 2] + pixels[i * 3 + 2][j * 3 + 2];
                average = average / 9.0f;
                float dofavg = (dofs[i * 3][j * 3] + dofs[i * 3][j * 3] + dofs[i * 3][j * 3] +
                    dofs[i * 3][j * 3] + dofs[i * 3][j * 3] + dofs[i * 3][j * 3] +
                    dofs[i * 3][j * 3] + dofs[i * 3][j * 3] + dofs[i * 3][j * 3]) / 9.0f;
                //cout << dofavg << endl;
                if (botthreshold<dofavg &&dofavg<topthreshold) {
                    image.SetPixel(i, j, average);
                    //cout << "dd" << endl;
                }
                else {
                    image.SetPixel(i, j, averageblur);
                }
            }
        }
    ...
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            Vector3f averageblur = gaussianHpixel[i * 3][j * 3] + gaussianHpixel[i * 3 + 1][j * 3] + gaussianHpixel[i * 3 + 2][j * 3] +
