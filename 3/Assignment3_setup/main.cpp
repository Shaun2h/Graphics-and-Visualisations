...
namespace
{
  ...
  void initSystem(int argc, char * argv[])
  {
	...
	for (int i = 1; i < argc; i++) {
		...
		if (i == 2) {
			...
			if (*argv[i] == 's') {
				system = new SimpleSystem();
				cout << "Simple System activated. " << argv[i] << endl;
			}
			else if (*argv[i] == 'p') {
				system = new PendulumSystem(4); //uhh 4 particles?
				cout << "Pendulum system activated"<< endl;
			}
			else if (*argv[i] == 'c') {
				system = new ClothSystem(64, 8); //uhh 400 particles?
				cout << "Cloth system activated" << endl;
			}
			
		}
		if (i == 3) {
			stepsize = atof(argv[i]);
			cout << "applied new stepsize: " << argv[i] << endl;
		}
	}
	if (stepsize < 0) {
		cout << "invalid stepsize" << endl;
		exit(0);
	}
	else if (stepsize == 0) {
		cout << "Proceeding with default stepsize of 0.04" << endl;
		stepsize = 0.04;
	}
  ...
  void drawSystem()
  {
    ...
    //glutSolidSphere(0.1f,10.0f,10.0f); //the central 0 0 0 sphere that i don't know what to do with.
    ...
    void keyboardFunc( unsigned char key, int x, int y )
    {
		...
		case 'r':
		{
			if (original == 's') {
				...
				system = new ClothSystem(64,8); //uhh 4 particles?
			}
			cout << "RESET" << endl;
			break;
		}
